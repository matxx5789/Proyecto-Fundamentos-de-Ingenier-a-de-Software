...
public class OrderService {
    ...
    public Page<OrderResponse> getHistory(Long buyerId, OrderStatus status, int page, int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        if (status == null) {
            return orderRepository.findByBuyerId(buyerId, pageable).map(this::toResponse);
        }
        return orderRepository.findByBuyerIdAndStatus(buyerId, status, pageable).map(this::toResponse);
    }
