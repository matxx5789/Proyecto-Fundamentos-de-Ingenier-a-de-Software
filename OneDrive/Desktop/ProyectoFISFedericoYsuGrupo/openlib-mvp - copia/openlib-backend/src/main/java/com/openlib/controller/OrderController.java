...
public class OrderController {
    ...
    @GetMapping
    @Operation(summary = "Historial de órdenes del usuario, opcionalmente filtrado por estado")
    public Page<OrderResponse> history(
            @AuthenticationPrincipal User user,
            @RequestParam(defaultValue = "0")  int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(required = false) OrderStatus status) {
        return orderService.getHistory(user.getId(), status, page, size);
    }
    ...
    @Operation(summary = "Cancelar una orden propia")
    public OrderResponse cancelOrder(@AuthenticationPrincipal User user,
                                     @PathVariable Long id) {
