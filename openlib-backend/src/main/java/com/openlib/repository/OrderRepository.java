...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    // Métricas de órdenes por estado en un período
    @Query("SELECT COUNT(o) FROM Order o WHERE o.status = :status AND o.createdAt >= :startDate AND o.createdAt <= :endDate")
    long countByStatusInPeriod(@Param("status") OrderStatus status, @Param("startDate") java.time.LocalDateTime startDate, @Param("endDate") java.time.LocalDateTime endDate);
}
