...
public class AdminController {
    ...
    @GetMapping("/orders")
    @Operation(summary = "Listar todas las órdenes del sistema")
    public Page<OrderResponse> orders(@RequestParam(defaultValue = "0")  int page,
                                      @RequestParam(defaultValue = "20") int size) {
        return orderService.getAllOrdersForAdmin(page, size);
    }

    @PutMapping("/orders/{id}/status/{newStatus}")
    @Operation(summary = "Cambiar estado de una orden (admin)")
    public OrderResponse changeOrderStatus(@PathVariable Long id,
                                           @PathVariable String newStatus) {
        return orderService.adminChangeOrderStatus(id, newStatus);
    }
    ...
    public Page<BookResponse> books(@RequestParam(defaultValue = "0")  int page,
        ...
        return bookService.getAllForAdmin(page, size, status);
    }
