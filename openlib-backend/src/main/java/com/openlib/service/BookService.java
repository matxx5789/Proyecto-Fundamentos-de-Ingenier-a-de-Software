...
public class BookService implements BookServicePort {
    ...
    public Page<BookResponse> getAllForAdmin(int page, int size, String status) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        
        // Si no hay status especificado o es vacío, traer todos
        ...
        try {
            ...
            return bookRepository.findByStatus(bookStatus, pageable).map(this::toResponse);
        } catch (IllegalArgumentException e) {
            // Si el estado es inválido, retornar vacío
